...
func TestDayFourPuzzleOne(t *testing.T) {
	...
	wordValidator := func(passphraseList string) int {

		validCount := 0
		for _, passphrase := range strings.Split(passphraseList, "\n") {
			wordBlocks := strings.Split(passphrase, " ")
			wordMap := make(map[string]bool)
			for _, word := range wordBlocks {
				wordMap[word] = true
			}
			if len(wordBlocks) == len(wordMap) {
				validCount++
			}
		}
	...
	assert.Equal(t, 1, wordValidator("aa bb cc dd ee"))
	assert.Equal(t, 0, wordValidator("aa bb cc dd aa"))
	assert.Equal(t, 1, wordValidator("aa bb cc dd aaa"))
	assert.Equal(t, 477, wordValidator(puzzleInput))
...
func TestDayFourPuzzleTwo(t *testing.T) {
	...
	anagramWordValidator := func(passphraseList string) int {
		validCount := 0
		for _, passphrase := range strings.Split(passphraseList, "\n") {
			wordBlocks := strings.Split(passphrase, " ")
			wordMap := make(map[string]bool)
			for _, word := range wordBlocks {
				detachedStrings := strings.Split(word, "")
				sort.Strings(detachedStrings)
				sortedRunes := strings.Join(detachedStrings, "")
				wordMap[sortedRunes] = true
			}
			if len(wordBlocks) == len(wordMap) {
				validCount++
			}
	...
	assert.Equal(t, 1, anagramWordValidator("abcde fghij"))
	assert.Equal(t, 0, anagramWordValidator("abcde xyz ecdab"))
	assert.Equal(t, 1, anagramWordValidator("a ab abc abd abf abj"))
	assert.Equal(t, 1, anagramWordValidator("iiii oiii ooii oooi oooo"))
	assert.Equal(t, 0, anagramWordValidator("oiii ioii iioi iiio"))
	assert.Equal(t, 167, anagramWordValidator(puzzleInput))

}

var puzzleInput = `una bokpr ftz ryw nau yknf fguaczl anu
