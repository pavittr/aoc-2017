...
func jump(input []int, offset int) ([]int, int) {
	nextOffset := offset + input[offset]
	input[offset]++
	return input, nextOffset
}

func weirdJump(input []int, offset int) ([]int, int) {
	jumpValue := input[offset]
	...
	nextOffset := offset + jumpValue
	return input, nextOffset
}

func TestDayFivePuzzleOne(t *testing.T) {

	jumpCalc := func(input []int) int {
		jumpCount := 1
		localArray := make([]int, len(input))
		for index, elem := range input {
			localArray[index] = elem
		}
		newArray, nextOffset := jump(localArray, 0)
		for nextOffset < len(input) {
			jumpCount++
			newArray, nextOffset = jump(newArray, nextOffset)
		}
		return jumpCount
	}

	assert.Equal(t, 5, jumpCalc([]int{0, 3, 0, 1, -3}))
	assert.Equal(t, 375042, jumpCalc(day5PuzzleInput))

}

func printArray(inputArray []int, offset int) {
	for i := 0; i < offset; i++ {
		fmt.Printf(" %+5d ", inputArray[i])
	}
	fmt.Printf("(%+5d)", inputArray[offset])
	for i := offset + 1; i < len(inputArray); i++ {
...
func TestDayFivePuzzleTwo(t *testing.T) {
	...
	jumpCalc := func(input []int) int {
		jumpCount := 1
		newArray, nextOffset := weirdJump(input, 0)
		for nextOffset < len(input) {
			//		printArray(newArray, nextOffset)
			jumpCount++
			newArray, nextOffset = weirdJump(newArray, nextOffset)
		}
		return jumpCount
	}

	assert.Equal(t, 10, jumpCalc([]int{0, 3, 0, 1, -3}))
	//fmt.Printf("%+v\n", jumpCalc(day5PuzzleInput))
	assert.Equal(t, 28707598, jumpCalc(day5PuzzleInput))

}

func TestWeirdJump(t *testing.T) {
	// 2 3 2 3 -1

	//  0 - (0)  3   0   1   -3
	//  1 - (1)  3   0   1   -3
	newArray, newOffset := weirdJump([]int{0, 3, 0, 1, -3}, 0)
	assert.Equal(t, []int{1, 3, 0, 1, -3}, newArray)
	assert.Equal(t, 0, newOffset)

	//  1 - (1)  3   0   1   -3
	//  2 -  2  (3)  0   1   -3
	newArray, newOffset = weirdJump([]int{1, 3, 0, 1, -3}, newOffset)
	assert.Equal(t, []int{2, 3, 0, 1, -3}, newArray)
	assert.Equal(t, 1, newOffset)

	//  2 -  2  (3)  0   1   -3
	//  3 -  2   2   0   1  (-3)
	newArray, newOffset = weirdJump([]int{2, 3, 0, 1, -3}, newOffset)
	assert.Equal(t, []int{2, 2, 0, 1, -3}, newArray)
	assert.Equal(t, 4, newOffset)

	//  3 -  2   2   0   1  (-3)
	//  4 -  2  (2)  0   1   -2
	newArray, newOffset = weirdJump([]int{2, 2, 0, 1, -3}, newOffset)
	assert.Equal(t, []int{2, 2, 0, 1, -2}, newArray)
	assert.Equal(t, 1, newOffset)

	//  4 -  2  (2)  0   1   -2
	//  5 -  2   3   0  (1)  -2
	newArray, newOffset = weirdJump([]int{2, 2, 0, 1, -2}, newOffset)
	assert.Equal(t, []int{2, 3, 0, 1, -2}, newArray)
	assert.Equal(t, 3, newOffset)

	//  5 -  2   3   0  (1)  -2
	//  6 -  2   3   0   2  (-2)
	newArray, newOffset = weirdJump([]int{2, 3, 0, 1, -2}, newOffset)
	assert.Equal(t, []int{2, 3, 0, 2, -2}, newArray)
	assert.Equal(t, 4, newOffset)

	//  6 -  2   3   0   2  (-2)
	//  7 -  2   3  (0)  2   -1
	newArray, newOffset = weirdJump([]int{2, 3, 0, 2, -2}, newOffset)
	assert.Equal(t, []int{2, 3, 0, 2, -1}, newArray)
	assert.Equal(t, 2, newOffset)

	//  7 -  2   3  (0)  2   -1
	//  8 -  2   3  (1)  2   -1
	newArray, newOffset = weirdJump([]int{2, 3, 0, 2, -1}, newOffset)
	assert.Equal(t, []int{2, 3, 1, 2, -1}, newArray)
	assert.Equal(t, 2, newOffset)

	//  8 -  2   3  (1)  2   -1
	//  9 -  2   3   2  (2)  -1
	newArray, newOffset = weirdJump([]int{2, 3, 1, 2, -1}, newOffset)
	assert.Equal(t, []int{2, 3, 2, 2, -1}, newArray)
	assert.Equal(t, 3, newOffset)

	//  9 -  2   3   2  (2)  -1
	// 10 -  2   3   2   3   -1
	newArray, newOffset = weirdJump([]int{2, 3, 2, 2, -1}, newOffset)
	assert.Equal(t, []int{2, 3, 2, 3, -1}, newArray)
